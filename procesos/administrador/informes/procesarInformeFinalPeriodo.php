...
// obtener los periodos de una anio
@$anio=$_POST['anioEleg'];
if(isset($anio)){
	$respuesta="";
	 $idCentro=$_SESSION['CentroEducativo'];
	   $datos=mysql_query("SELECT c.periodo FROM grupo g
		 ...
		 while($reg=mysql_fetch_array($datos)){
			 $periodo=$reg['periodo'];
			 $respuesta.="<option value='$periodo'>$periodo</option>";
		 } 
	echo $respuesta;
}

// cargar los grupos de un año y perido
@$perdiodo=$_POST['periodoEleg'];
if(isset($perdiodo)){
	$anio=$_POST['anioPerEleg'];
	...
	$respuesta="";
	 $idCentro=$_SESSION['CentroEducativo'];
	   $datos=mysql_query("SELECT g.id,g.nombre FROM grupo g
	                       ...
	                       WHERE g.centroeducativo='$idCentro' AND c.anio='$anio' AND c.periodo='$perdiodo'
						   GROUP by c.grupo
						   ORDER BY gr.numero
	...
	$respuesta="";
	 $idCentro=$_SESSION['CentroEducativo'];
	   ...
	   $datos=mysql_query("SELECT m.id,m.nombres,m.apellidos FROM grupo g
	                       ...
	                       WHERE g.centroeducativo='$idCentro' AND c.anio='$anio' AND c.periodo='$periodo' AND c.grupo='$grupo'
						   GROUP by m.id
	                        ");
		 while($reg=mysql_fetch_array($datos)){
			 $idEst=$reg['id'];
		 ...
		 while($reg=mysql_fetch_array($datos)){
			 ...
			 echo '
				<table border="1" cellspacing="0" style="width:100%">
				 ...
				 <td>Peridodo:</td>
				 <td><span class="spnNegrita">'.$periodoInf.'</span></td>
				 <td>Id:</td>
			 ...
			 $porciones = mb_str_split($nomArea);
			  ...
			  <td class="tdsInfoAreas">
		        ...
		        while($reg2=mysql_fetch_array($datos2)){
				  ...
				  $datos3=mysql_query("SELECT falta FROM faltas WHERE matricula='$estInf' AND materia='$materia' AND periodo='$periodoInf' AND anio='$anio' ");
				  ...
				  <tr>
					...
					<td>Valoración Asignatura</td>
					<td>Valor Acumulado Anual</td>
				  </tr>
				  <tr>
					...
					$datos3=mysql_query("SELECT nombre FROM indicadorlogro WHERE grado='$grado' AND periodo='$periodoInf' AND materia='$materia' AND estado='si' ORDER BY fechaModificacion ");
					while($reg3=mysql_fetch_array($datos3)){
						$nomInd=$reg3['nombre'];
						echo " <li>$nomInd</li>";
					}
					...
					$notaFinal=notaFinalEst($estInf,$materia,$anio,$periodoInf);
					
					$noteParaRec=0;
					if($notaFinal<$notaInf){
						 $datos2I=mysql_query("SELECT m.notarecuperada  FROM informerecuperacion i 
											   ...
											   WHERE  i.grupo='$grupo' AND i.anio='$anio' AND i.periodo='$periodoInf' 
											   AND i.materia='$materia' AND m.matricula='$estInf'");
					...
					$datosConceptoM=mysql_query("SELECT sum(porcentaje) as total FROM conceptoevaluacion WHERE grupo='$grupo' AND materia='$materia' AND anio='$anio' AND periodo='$periodoInf'");
					while($regConceptoM=mysql_fetch_array($datosConceptoM)){
						$porcentEvaluadoMat=($regConceptoM['total']==NULL)?0:$regConceptoM['total'];
					}
					$acomuladaAnio=acomuladaAnio($estInf,$materia,$anio,$periodoInf,$notaFinal,$notaInf,$grupo);
					
					$notaFinal=($notaFinal>$noteParaRec)?$notaFinal:$noteParaRec;
					$porcentDivi=($porcentEvaluadoMat==0)?1:$porcentEvaluadoMat;
					$notaFinal=$notaFinal*100/$porcentDivi;
					$notaPorcentual=$notaFinal*$porcentajeMat/100;
					$notaPorcentual=round($notaPorcentual,1);
					
					
					$acomuladaArea+=$notaPorcentual;
					$notaFinal=round($notaFinal,1);
					$acomuladaAnio=round($acomuladaAnio,3);
					
					echo'<td class="tdsNotasAsing">'.$notaFinal.'<br /><span class="spnPorcentajeAsig" >('.$porcentajeMat.'%)</span></td>
					<td class="tdsAcomuladaAnio">'.$acomuladaAnio.'</td>
				  </tr>';
		         }
...
function acomuladaAnio($idest,$materia,$anio,$periodo,$notaPer,$nInf,$grupoE){
	$acomulada=0;
	 if (file_exists('../../../sistemaEvaluativo/sistemaEvaluativo.xml')){
	    $xml=simplexml_load_file("../../../sistemaEvaluativo/sistemaEvaluativo.xml", 'SimpleXMLElement',LIBXML_NOCDATA);
